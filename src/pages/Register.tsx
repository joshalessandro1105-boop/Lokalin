...
const Register = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name || !email || !password || !confirmPassword || !phone || !location) {
      setError("Isi semua kolom agar bisa mendaftar.");
      return;
    }
      ...
      <main className="container mx-auto px-4 py-16">
        <section className="mx-auto max-w-3xl rounded-3xl border border-border bg-card/95 p-8 shadow-xl shadow-black/5 backdrop-blur-xl">
          ...
          <form onSubmit={handleSubmit} className="grid gap-6 rounded-3xl border border-border bg-background/80 p-6 shadow-sm">
            ...
            <Button type="submit" className="w-full rounded-full bg-gradient-primary px-6 py-3 font-semibold text-primary-foreground hover:opacity-95">
              Daftar Sekarang
            </Button>
